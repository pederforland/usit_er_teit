...
def evaluate_batch(predicted_labels: List[int], gold_labels: List[int]) -> dict:
    """Evaluate a batch of predictions against gold labels.

    Returns a dict with 'accuracy' and 'krippendorff_alpha'.
    """
    if len(predicted_labels) == 0:
        raise ValueError("Empty input: predicted_labels and gold_labels must be non-empty")

    if len(predicted_labels) != len(gold_labels):
        raise ValueError(
            f"Length mismatch: {len(predicted_labels)} predictions vs {len(gold_labels)} gold labels"
        )

    invalid_preds = set(predicted_labels) - VALID_LABELS
    if invalid_preds:
        raise ValueError(
            f"Invalid predicted labels: {invalid_preds}. Labels must be in {sorted(VALID_LABELS)}"
        )

    invalid_golds = set(gold_labels) - VALID_LABELS
    if invalid_golds:
        raise ValueError(
            f"Invalid gold labels: {invalid_golds}. Labels must be in {sorted(VALID_LABELS)}"
        )

    accuracy = sum(p == g for p, g in zip(predicted_labels, gold_labels)) / len(gold_labels)

    data = [gold_labels, predicted_labels]
    ka = alpha(
    ...
    return {"accuracy": accuracy, "krippendorff_alpha": ka}


if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(f"Usage: python3 {sys.argv[0]} pred.jsonl gold.jsonl", file=sys.stderr)
        sys.exit(1)

    pred_path, gold_path = sys.argv[1], sys.argv[2]

    with open(pred_path) as f:
        predicted_labels = [json.loads(line)["median_label"] for line in f]

    with open(gold_path) as f:
        gold_labels = [json.loads(line)["median_label"] for line in f]

    results = evaluate_batch(predicted_labels, gold_labels)
